...
function App() {
  ...
  const stageStatusLabel = screenShareOn
    ? "SCREEN LIVE"
    : cameraOn
    ? "CAMERA LIVE"
    : "STAGE READY";

  const stageStatusStyle = screenShareOn
    ? styles.stageStatusPresenting
    : cameraOn
    ? styles.stageStatusLive
    : styles.stageStatusReady;
  ...
  useEffect(() => {
    ...
    if (cameraOn && screenShareOn && localCameraStream) {
      if (floatingVideo.srcObject !== localCameraStream) {
        ...
        applyAudioOutput(floatingVideo);
      }
      return;
    }
  ...
  async function enumerateMediaDevices() {
    try {
      ...
      if (!selectedMicrophoneId && microphones[0]?.deviceId) setSelectedMicrophoneId(microphones[0].deviceId);
      if (!selectedSpeakerId && speakers[0]?.deviceId) setSelectedSpeakerId(speakers[0].deviceId);
    } catch (error) {
      console.log(error);
    }
  }

  async function applyAudioOutput(target) {
    if (!target || !selectedSpeakerId || typeof target.setSinkId !== "function") return;

    try {
      await target.setSinkId(selectedSpeakerId);
    } catch (error) {
      console.log(error);
    }
  }
  ...
  async function handleTestCamera() {
    try {
      ...
      setStatusText("Camera device ready");
      await enumerateMediaDevices();
    } catch (error) {
      setStatusText("Selected camera is not available");
      alert("Selected camera could not be opened. Try another camera device.");
    }
  }
        ...
        {showWelcomeOverlay && (
          <div style={styles.welcomeOverlay}>
            <div style={styles.welcomeCard}>
              <div style={styles.welcomeEyebrow}>Avant Global Vision</div>
              <div style={styles.welcomeTitle}>Welcome to the premium broadcast floor</div>
              <div style={styles.welcomeText}>
                Select a room, start your stage, and engage your audience with a polished live experience.
              </div>
              <div style={styles.welcomeList}>
                <div>• Pick a room and enter the stage.</div>
                <div>• Use Control Center to manage hosts, moderators, and room settings.</div>
                <div>• Keep your camera, mic, and speaker choices tuned from one place.</div>
              </div>
              <button style={styles.primaryButton} onClick={() => setShowWelcomeOverlay(false)}>
                Enter Platform
              </button>
            </div>
          </div>
            ...
            <div style={styles.stageCard}>
              ...
              <div style={styles.stageHeader}>
                ...
                <div style={styles.stageMetaWrap}>
                  <div style={stageStatusStyle}>{stageStatusLabel}</div>
                  <div style={styles.metaPill}>Host: {selectedRoom?.assignedHost || selectedRoom?.host || "Admin"}</div>
                  ...
                  roomParticipants.map((person, index) => (
                    <div key={`${person.sessionId || person.name}-${index}`} style={styles.participantCard}>
                      ...
                      <div>
                        ...
                        <div style={styles.participantRoleRow}>
                          <div style={styles.participantRole}>
                            {person.sessionId === clientSessionId ? "You" : person.role || "Participant"}
                          </div>
                          <div style={roleBadgeStyle(person.role, person.name === authUser.displayName ? authUser.globalRole : "")}>
                            {roleBadgeText(person.role, person.name === authUser.displayName ? authUser.globalRole : "")}
                          </div>
                        </div>
                      </div>
                    </div>
          ...
          {selectedPanel === "controls" && (
            <section style={styles.panelCard}>
              ...
              <div style={styles.controlGrid}>
                ...
                <div style={styles.controlBox}>
                  <div style={styles.controlBoxTitle}>Device Control</div>
                  <div style={styles.inlineColumn}>
                    <select style={styles.composerInput} value={selectedCameraId} onChange={(e) => setSelectedCameraId(e.target.value)}>
                      <option value="">Default camera</option>
                      {cameraDevices.map((device, index) => (
                        <option key={`cam-${device.deviceId || index}`} value={device.deviceId}>
                          {device.label || `Camera ${index + 1}`}
                        </option>
                      ))}
                    </select>
                    <select style={styles.composerInput} value={selectedMicrophoneId} onChange={(e) => setSelectedMicrophoneId(e.target.value)}>
                    ...
                    <select style={styles.composerInput} value={selectedSpeakerId} onChange={(e) => setSelectedSpeakerId(e.target.value)}>
                      <option value="">Default speaker</option>
                      {speakerDevices.map((device, index) => (
                        <option key={`spk-${device.deviceId || index}`} value={device.deviceId}>
                          {device.label || `Speaker ${index + 1}`}
                        </option>
                      ))}
                    </select>
                    <div style={styles.inlineRowWrap}>
                      <button style={styles.secondaryButton} onClick={enumerateMediaDevices}>Refresh Devices</button>
                      <button style={styles.primaryButton} onClick={handleTestCamera}>Test Camera</button>
                    </div>
                  </div>
                </div>
...
function roleBadgeText(role, globalRole) {
...
function roleBadgeStyle(role, globalRole) {
  if (globalRole === "superadmin") return styles.participantBadgeAdmin;
  if (role === "host") return styles.participantBadgeHost;
  if (role === "moderator") return styles.participantBadgeModerator;
  return styles.participantBadgeAudience;
}
...
const styles = {
  ...
  stageStatusPresenting: {
    ...
    fontSize: 12,
    fontWeight: 800,
    boxShadow: "0 0 22px rgba(46,168,255,0.16)",
  },
  welcomeOverlay: {
    marginBottom: 18,
    borderRadius: 24,
    ...
    background: "linear-gradient(180deg, rgba(46,168,255,0.12), rgba(255,255,255,0.04))",
  ...
  participantBadgeAdmin: {
    padding: "4px 8px",
    borderRadius: 999,
    background: "rgba(228,196,108,0.18)",
    border: "1px solid rgba(228,196,108,0.28)",
    fontSize: 11,
    fontWeight: 800,
    color: "#f4dd9a",
  },
  participantBadgeHost: {
    padding: "4px 8px",
    borderRadius: 999,
    background: "rgba(34,197,94,0.18)",
    border: "1px solid rgba(34,197,94,0.28)",
