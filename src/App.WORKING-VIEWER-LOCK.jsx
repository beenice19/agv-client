...
function App() {
  ...
  const userRole = useMemo(() => {
    ...
    if (selectedRoom.assignedHost === authUser.displayName || selectedRoom.host === authUser.displayName) return "host";
    if (Array.isArray(selectedRoom.moderators) && selectedRoom.moderators.includes(authUser.displayName)) return "moderator";
    return "viewer";
  ...
  const liveStatusText = screenShareOn ? "Presenting" : cameraOn ? "Camera Live" : "Standby";
  ...
  useEffect(() => {
    ...
    if (socketRef.current) {
      socketRef.current.emit("room:subscribe", { roomId: selectedRoomId, sessionId: clientSessionId });
    }

    return () => {
      if (socketRef.current) {
        socketRef.current.emit("room:unsubscribe", { roomId: selectedRoomId });
      }
    };
  ...
  useEffect(() => {
    ...
    if (cameraOn && localCameraStream) {
      if (stageVideo.srcObject !== localCameraStream) stageVideo.srcObject = localCameraStream;
      return;
    }
    ...
    const handleBeforeUnload = () => {
      try {
        navigator.sendBeacon(
          ...
          new Blob([JSON.stringify({ sessionId: clientSessionId })], { type: "application/json" })
        );
      } catch {
        // Safe no-op.
      }
  ...
  async function fetchMe() {
    try {
      const { res, data } = await fetchJson(`${API_BASE}/api/auth/me`, { method: "GET" });
  ...
  async function loadRooms() {
    try {
      const { res, data } = await fetchJson(`${API_BASE}/api/rooms`, { method: "GET" });
  ...
  async function loadRoomState(roomId) {
    try {
      const { res, data } = await fetchJson(`${API_BASE}/api/rooms/${encodeURIComponent(roomId)}/state`, { method: "GET" });
  ...
  function applyRoomState(roomId, state) {
    setMessagesByRoom((prev) => ({ ...prev, [roomId]: Array.isArray(state?.messages) ? state.messages : [] }));
    setBulletinItemsByRoom((prev) => ({ ...prev, [roomId]: Array.isArray(state?.bulletins) ? state.bulletins : [] }));
    setBulletinSourceByRoom((prev) => ({ ...prev, [roomId]: String(state?.bulletinSource || "manual") }));
  }
  ...
  async function joinPresence(roomId) {
    ...
    try {
      ...
      setParticipantsByRoom((prev) => ({ ...prev, [roomId]: Array.isArray(data.participants) ? data.participants : [] }));
  ...
  function connectSocket() {
    ...
    socket.on("connect", () => {
      ...
      socket.emit("room:subscribe", { roomId: selectedRoomId, sessionId: clientSessionId });
    });
    ...
    socket.on("presence:update", (payload) => {
      ...
      setParticipantsByRoom((prev) => ({ ...prev, [payload.roomId]: payload.participants }));
    });
    ...
    socket.on("room:snapshot", (payload) => {
      if (payload?.room) {
        ...
        setRooms((prev) => {
          ...
          return exists ? prev.map((room) => (room.id === normalized.id ? normalized : room)) : [...prev, normalized];
        });
      }
      ...
      if (Array.isArray(payload?.participants) && payload?.room?.id) {
        setParticipantsByRoom((prev) => ({ ...prev, [payload.room.id]: payload.participants }));
      }
    });
  }
  ...
  async function loadAdminUsers() {
    ...
    try {
      const { res, data } = await fetchJson(`${API_BASE}/api/admin/users`, { method: "GET" });
  ...
  async function handleCreateRoom() {
    ...
    try {
      const { res, data } = await fetchJson(`${API_BASE}/api/rooms`, {
        method: "POST",
        body: JSON.stringify({ name, category: "Custom", isPrivate: false }),
      });
  ...
  async function handleTogglePrivacy() {
    ...
    try {
      const { res, data } = await fetchJson(`${API_BASE}/api/rooms/${encodeURIComponent(selectedRoomId)}/privacy`, {
        method: "POST",
        body: JSON.stringify({ isPrivate: !selectedRoom.isPrivate }),
      });
  ...
  async function handleToggleLock() {
    ...
    try {
      const { res, data } = await fetchJson(`${API_BASE}/api/rooms/${encodeURIComponent(selectedRoomId)}/lock`, {
        method: "POST",
        body: JSON.stringify({ isLocked: !selectedRoom.isLocked }),
      });
  ...
  function buildCameraConstraints() {
    ...
    return { deviceId: { exact: selectedCameraDeviceId }, width: { ideal: 1280 }, height: { ideal: 720 } };
  }

  function buildMicConstraints() {
    if (!selectedMicDeviceId) return true;
    return { deviceId: { exact: selectedMicDeviceId }, echoCancellation: true, noiseSuppression: true, autoGainControl: true };
  }

  function handleSelectCameraDevice(deviceId) {
    setSelectedCameraDeviceId(deviceId);
    if (deviceId) window.localStorage.setItem(DEVICE_CAMERA_KEY, deviceId);
    else window.localStorage.removeItem(DEVICE_CAMERA_KEY);
    setStatusText("Camera selection saved. Restart camera to apply.");
  }

  function handleSelectMicDevice(deviceId) {
    setSelectedMicDeviceId(deviceId);
    if (deviceId) window.localStorage.setItem(DEVICE_MIC_KEY, deviceId);
    else window.localStorage.removeItem(DEVICE_MIC_KEY);
  ...
  async function handleToggleCamera() {
    ...
    try {
      ...
      setCameraOn(true);
      setStatusText("Camera live locally for host/admin");
  ...
  async function handleToggleScreenShare() {
    ...
    try {
      ...
      setStatusText("Screen sharing live locally for host/admin");
    } catch {
      ...
      <div style={styles.loginShell}>
        <div style={styles.loginCard}>
          ...
          <p style={styles.loginSubtitle}>
            {window.location.search.includes("room=") ? "Sign in to enter your invited room." : "Sign in to your broadcast platform."}
          </p>
    ...
    <div style={styles.appShell}>
      <header style={styles.header}>
        ...
        <div style={styles.headerRight}>
          ...
          <div style={styles.userPill}>{authUser.displayName} • {roleText}</div>
          <button style={styles.dangerButton} onClick={logout}>Log Out</button>
        </div>
      </header>
      ...
      <main style={styles.mainGrid}>
        <aside style={styles.leftPanel}>
          ...
          <div style={styles.roomList}>
            {filteredRooms.map((room) => (
              ...
              >
                ...
                <div style={styles.roomMeta}>{room.category} • {room.isPrivate ? "Private" : "Public"} • {room.isLocked ? "Locked" : "Open"}</div>
                <div style={styles.roomMeta}>Host: {room.assignedHost || room.host || "Admin"}</div>
              </button>
            ))}
          </div>
          ...
          {!isViewerOnly ? (
            <div style={styles.controlBox}>
              ...
              />
              <button style={styles.primaryButton} onClick={handleCreateRoom}>Create Room</button>
            </div>
          ) : (
        ...
        <section style={styles.centerPanel}>
          <div style={styles.identityCard}>
            <div>
              ...
              <div style={styles.identityLine}>Signed in as {authUser.displayName} • Room Role: {roleText}</div>
            </div>
            <div style={styles.identityChips}>
              ...
              <span style={styles.chip}>{selectedRoomParticipants.length} people</span>
            </div>
          </div>
          ...
          <div style={styles.stageShell}>
            ...
            <div style={styles.stageViewport}>
              {(cameraOn || screenShareOn) && canControlStage ? (
                <video ref={stageVideoRef} autoPlay playsInline muted style={styles.stageVideo} />
              ) : (
                <div style={styles.stagePlaceholder}>
                  ...
                  <div style={styles.stagePlaceholderText}>
                    {isViewerOnly
                      ? "You are in viewer mode. Chat and bulletin are available. Host controls, Google Drive, and Control Center are locked."
                      : "Start camera or screen share locally. Real viewer broadcast requires the next SERVER + CLIENT media transport pass."}
                  </div>
                </div>
              )}
            </div>
            ...
            {canControlStage ? (
              <div style={styles.stageControls}>
                <button style={cameraOn ? styles.activeButton : styles.primaryButton} onClick={handleToggleCamera}>
                  {cameraOn ? "Stop Camera" : "Start Camera"}
                </button>
                <button style={micOn ? styles.activeButton : styles.secondaryButton} onClick={handleToggleMic}>
                  {micOn ? "Mute Mic" : "Start Mic"}
                </button>
                <button style={screenShareOn ? styles.activeButton : styles.primaryButton} onClick={handleToggleScreenShare}>
                  {screenShareOn ? "Stop Share" : "Share Screen"}
                </button>
                <select style={styles.select} value={selectedCameraDeviceId} onChange={(event) => handleSelectCameraDevice(event.target.value)}>
                ...
                <select style={styles.select} value={selectedMicDeviceId} onChange={(event) => handleSelectMicDevice(event.target.value)}>
            ...
            <div style={styles.card}>
              ...
              <div style={styles.bulletinList}>
                {selectedRoomBulletins.length ? (
                  selectedRoomBulletins.map((item, index) => <div key={`${item}-${index}`} style={styles.bulletinItem}>{item}</div>)
                ) : (
              ...
              {canOperateRoom ? (
                <>
                  ...
                  <div style={styles.row}>
                    <button style={styles.primaryButton} onClick={handleAnnouncementAdd}>Add Bulletin</button>
                    <button style={styles.secondaryButton} onClick={() => bulletinFileRef.current?.click()}>Import Text</button>
                  </div>
                  <input ref={bulletinFileRef} type="file" accept=".txt,text/plain" style={{ display: "none" }} onChange={handleBulletinUpload} />
                </>
              ) : (
        ...
        <aside style={styles.rightPanel}>
          <div style={styles.tabRow}>
            <button style={selectedPanel === "chat" ? styles.tabActive : styles.tab} onClick={() => setSelectedPanel("chat")}>Chat</button>
            <button style={selectedPanel === "bulletin" ? styles.tabActive : styles.tab} onClick={() => setSelectedPanel("bulletin")}>Bulletin</button>
            {!isViewerOnly ? (
              <button style={selectedPanel === "controls" ? styles.tabActive : styles.tab} onClick={() => setSelectedPanel("controls")}>Control</button>
            ) : null}
          </div>
          ...
          {selectedPanel === "chat" && (
            <div style={styles.card}>
                ...
                {selectedRoomMessages.length ? (
                  selectedRoomMessages.map((message, index) => (
                    <div key={message.id || index} style={styles.chatMessage}>
                      <div style={styles.chatMeta}>{message.sender || "User"} • {message.time || ""}</div>
              ...
              <div style={styles.chatComposer}>
                ...
                />
                <button style={styles.primaryButton} onClick={handleSendMessage}>Send</button>
              </div>
            </div>
          )}
          ...
          {selectedPanel === "bulletin" && (
            <div style={styles.card}>
              ...
              <div style={styles.helperText}>Source: {bulletinSourceByRoom[selectedRoomId] || "manual"}</div>
              <div style={styles.bulletinListTall}>
                {selectedRoomBulletins.length ? (
                  selectedRoomBulletins.map((item, index) => <div key={`${item}-feed-${index}`} style={styles.bulletinItem}>{item}</div>)
                ) : (
                  <div style={styles.emptyText}>No bulletins loaded.</div>
                )}
              </div>
          ...
          {selectedPanel === "controls" && !isViewerOnly && (
            <div style={styles.card}>
              ...
              <button style={styles.primaryButton} onClick={copyInviteLink}>Copy Invite Link</button>
              <button style={styles.secondaryButton} onClick={handleOpenGoogleDrive}>Open Google Drive</button>

              <div style={styles.controlBox}>
                <div style={styles.controlTitle}>Room Settings</div>
                <button style={styles.secondaryButton} onClick={handleTogglePrivacy}>{selectedRoom?.isPrivate ? "Set Public" : "Set Private"}</button>
                <button style={styles.secondaryButton} onClick={handleToggleLock}>{selectedRoom?.isLocked ? "Unlock Room" : "Lock Room"}</button>
              </div>
              ...
              {isSuperadmin ? (
                <>
                  <div style={styles.controlBox}>
                    ...
                    <input style={styles.input} value={hostInput} onChange={(event) => setHostInput(event.target.value)} placeholder="Display name" />
                    <button style={styles.primaryButton} onClick={handleAssignHost}>Assign Host</button>
                  </div>

                  <div style={styles.controlBox}>
                    <div style={styles.controlTitle}>Create User</div>
                    <input style={styles.input} value={createUserUsername} onChange={(event) => setCreateUserUsername(event.target.value)} placeholder="Username" />
                    <input style={styles.input} value={createUserDisplayName} onChange={(event) => setCreateUserDisplayName(event.target.value)} placeholder="Display name" />
                    <input style={styles.input} type="password" value={createUserPassword} onChange={(event) => setCreateUserPassword(event.target.value)} placeholder="Password" />
                    <select style={styles.select} value={createUserRole} onChange={(event) => setCreateUserRole(event.target.value)}>
                      <option value="user">User</option>
                      <option value="superadmin">Superadmin</option>
                    </select>
                    <button style={styles.primaryButton} onClick={handleCreateUser}>Create User</button>
                  </div>
                    ...
                    {adminUsers.map((user) => (
                      <div key={user.username} style={styles.userRow}>
                        ...
                        <button
                          ...
                          onClick={() => user.isActive ? handleDeactivateUser(user.username) : handleReactivateUser(user.username)}
                        >
              ...
              {canOperateRoom ? (
                <div style={styles.controlBox}>
                  ...
                  <input style={styles.input} value={moderatorInput} onChange={(event) => setModeratorInput(event.target.value)} placeholder="Display name" />
                  <button style={styles.primaryButton} onClick={handleAddModerator}>Add Moderator</button>
                  {(selectedRoom?.moderators || []).map((name) => (
                    <div key={name} style={styles.userRow}>
                      <span>{name}</span>
                      <button style={styles.smallDangerButton} onClick={() => handleRemoveModerator(name)}>Remove</button>
                    </div>
                  ))}
                </div>
              ) : null}
          ...
          {isViewerOnly ? (
            <div style={styles.viewerLockBox}>
              Viewer Mode Active: Control Center, Google Drive, stage camera, mic, screen share, host assignment, room creation, and bulletin editing are locked.
            </div>
          ) : null}
        </aside>
      </main>
...
const styles = {
  ...
  stageVideo: { width: "100%", height: "100%", objectFit: "cover", background: "#020617" },
  stagePlaceholder: {
    height: "100%",
    display: "flex",
    alignItems: "center",
  ...
  bottomGrid: { display: "grid", gridTemplateColumns: "1fr 1.2fr", gap: 16 },
  card: {
  ...
  panelTitle: { fontSize: 15, fontWeight: 950, marginBottom: 12, letterSpacing: 0.2 },
  roomList: { display: "grid", gap: 10, marginTop: 12 },
  roomButton: {
    textAlign: "left",
    padding: 12,
    borderRadius: 16,
  ...
  roomName: { fontWeight: 900, marginBottom: 4 },
  roomMeta: { fontSize: 12, color: "rgba(248,250,252,0.58)", lineHeight: 1.45 },
  input: {
    width: "100%",
  ...
  chatMeta: { color: "rgba(248,250,252,0.58)", fontSize: 12, fontWeight: 800, marginBottom: 4 },
  chatComposer: { display: "flex", gap: 8, marginTop: 12 },
  chatInput: {
    flex: 1,
    padding: "11px 12px",
    borderRadius: 14,
  ...
  bulletinList: { display: "grid", gap: 8, maxHeight: 220, overflowY: "auto" },
  bulletinListTall: { display: "grid", gap: 8, height: 440, overflowY: "auto" },
  bulletinItem: {
    padding: 11,
    borderRadius: 14,
  ...
  controlTitle: { fontWeight: 950, marginBottom: 10 },
  userRow: {
    display: "flex",
    justifyContent: "space-between",
    gap: 8,
  ...
  emptyText: { color: "rgba(248,250,252,0.58)", fontSize: 13, lineHeight: 1.5 },
  helperText: { color: "rgba(248,250,252,0.58)", fontSize: 13, lineHeight: 1.5, marginBottom: 10 },
  viewerLockBox: {
    marginTop: 12,
    padding: 12,
    borderRadius: 16,
  ...
  loginTitle: { margin: "0 0 8px", fontSize: 30, fontWeight: 950 },
  loginSubtitle: { margin: "0 0 20px", color: "rgba(248,250,252,0.62)", lineHeight: 1.5 },
  loginInput: {
    width: "100%",
    boxSizing: "border-box",
    padding: "13px 14px",
