...
function App() {
  ...
  const [inviteRoomId, setInviteRoomId] = useState(() => getInviteRoomFromUrl());
  const [inviteRoomMissing, setInviteRoomMissing] = useState(false);
  ...
  function applyInitialRoomFromUrl(roomList) {
    const roomFromUrl = getInviteRoomFromUrl();
    setInviteRoomId(roomFromUrl || "");

    if (roomFromUrl) {
      const matchedRoom = roomList.find((room) => room.id === roomFromUrl);

      if (matchedRoom) {
        setSelectedRoomId(matchedRoom.id);
        setOpenedFromInvite(true);
        setInviteRoomMissing(false);
        setStatusText(`Invite link opened: ${matchedRoom.name}`);
        return;
      }

      setOpenedFromInvite(false);
      setInviteRoomMissing(true);
      setStatusText(`Invite room not found: ${roomFromUrl}`);
    } else {
      setOpenedFromInvite(false);
      setInviteRoomMissing(false);
    }
      ...
      <div style={loginStyles.shell}>
        <div style={loginStyles.card}>
          ...
          <div style={loginStyles.subtitle}>
            {inviteRoomId
              ? `Sign in to enter your invited room${inviteRoomMissing ? ` (${inviteRoomId})` : ""}`
              : "Sign in to Avant Global Vision"}
          </div>
  ...
  return (
    <div style={styles.appShell}>
      ...
      <main style={styles.main}>
        ...
        {!openedFromInvite && inviteRoomId && inviteRoomMissing && (
          <section
            style={{
          ...
          >
            <div style={styles.inviteEntryTitle}>Invite room unavailable</div>
            <div style={styles.inviteEntryText}>
              The room link <strong>{inviteRoomId}</strong> is not available right now. Sign in and choose another room or ask for a new invite.
            </div>
          </section>
        )}
