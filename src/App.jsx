...
function TicketAdminPanel({ onBack }) {
  ...
  async function createTicket() {
    ...
    try {
      ...
      if (!data.ok) {
        setMessage(data.message || "Unable to create ticket.");
        setWorking(false);
        return;
      }
      ...
      setMessage("Unable to reach AGV ticket server.");
    }

    setWorking(false);
  }

  async function loadTickets(showSuccess = true) {
    setWorking(true);
    setMessage("");
    ...
    try {
      ...
      if (showSuccess) setMessage("Ticket list loaded.");
    } catch (error) {
  ...
  return (
    <div style={styles.page}>
      ...
      <main style={styles.adminShell}>
        <div style={styles.adminHeader}>
          <div>
            <div style={styles.badge}>AGV SALES ADMIN</div>
            <h1 style={styles.ctaTitle}>Ticket Sales Panel</h1>
            <p style={styles.ctaText}>
              Create paid access tickets, copy ticket codes, and review issued event access.
            </p>
          </div>
        ...
        <section style={styles.adminGrid}>
          <div style={styles.adminCard}>
            <h2 style={styles.cardTitle}>Create Ticket</h2>

            <label style={styles.adminLabel}>Admin PIN</label>
            <input
              value={adminPin}
            ...
            <label style={styles.adminLabel}>Buyer Name</label>
            <input
              value={buyerName}
              onChange={(e) => setBuyerName(e.target.value)}
              placeholder="Example: Jane Guest"
              style={styles.adminInput}
            />

            <label style={styles.adminLabel}>Buyer Email</label>
            <input
              value={buyerEmail}
            ...
            <label style={styles.adminLabel}>Event Name</label>
            <input
              value={eventName}
              onChange={(e) => setEventName(e.target.value)}
              placeholder="AGV Live Event"
              style={styles.adminInput}
            />

            <label style={styles.adminLabel}>Room ID</label>
            <input
              value={roomId}
            ...
            <div style={styles.buttonRow}>
              ...
              <button style={styles.secondaryButton} onClick={() => loadTickets(true)}>
                Load Tickets
              </button>
            </div>

            {message ? <p style={styles.adminMessage}>{message}</p> : null}
          </div>

          <div style={styles.adminCard}>
            <h2 style={styles.cardTitle}>Latest Ticket</h2>

            {createdTicket ? (
              <div style={styles.ticketResult}>
                <div style={styles.ticketCode}>{createdTicket.code}</div>
                <div style={styles.ticketMeta}>
                  {createdTicket.buyerName} · {createdTicket.eventName}
                </div>
                <button
                  style={styles.secondaryButton}
                  onClick={() => navigator.clipboard?.writeText(createdTicket.code)}
                >
                  Copy Code
                </button>
              </div>
              ...
              <p style={styles.cardText}>
                Create a ticket and the new code will appear here for copying.
              </p>
            )}
          </div>
        </section>
        ...
        <section style={styles.adminCard}>
          <h2 style={styles.cardTitle}>Issued Tickets</h2>

          {tickets.length === 0 ? (
            <p style={styles.cardText}>No tickets loaded yet.</p>
          ) : (
            <div style={styles.ticketTable}>
              {tickets.map((ticket) => (
                <div style={styles.ticketRow} key={ticket.code}>
                  <div>
                    <strong>{ticket.code}</strong>
                    <div style={styles.ticketMeta}>
                      {ticket.buyerName || "Guest"} · {ticket.eventName || "AGV Live Event"}
                    </div>
                  </div>
                  <div style={styles.ticketStatus}>
                    {ticket.used ? "Used" : "Open"}
                  </div>
                </div>
              ))}
            </div>
    ...
    <div style={styles.page}>
      ...
      <header style={styles.nav}>
        ...
        <div style={styles.navActions}>
          <button style={styles.navButton} onClick={onAdmin}>
            Ticket Admin
          </button>
      ...
      <main style={styles.shell}>
        ...
        <section style={styles.section}>
          ...
          <div style={styles.grid3}>
            <Card title="Churches & Ministries" text="Host services, conferences, revivals, teaching sessions, and private leadership rooms." />
            <Card title="Promoters & Event Hosts" text="Sell access to live shows, panels, conventions, seminars, and exclusive online events." />
            <Card title="Educators & Coaches" text="Run paid classes, workshops, training events, and group teaching sessions." />
          </div>
        </section>
        ...
        <section style={styles.grid3}>
          <Card title="Stage-first experience" text="The audience sees a professional stage instead of a basic meeting grid." />
          <Card title="Viewer-safe controls" text="Guests can watch and participate without access to host tools." />
          <Card title="Digital venue model" text="Rooms can represent halls, studios, classes, lounges, and private event spaces." />
        </section>
