...
export default function App() {
  ...
  const [displayName, setDisplayName] = useState("Host User");
  const [userRole, setUserRole] = useState("host");
  const [leftPanelTab, setLeftPanelTab] = useState("rooms");
  ...
  const [roomFormVisibility, setRoomFormVisibility] = useState("public");
  const [roomFormAccessCode, setRoomFormAccessCode] = useState("");
  const [roomAccessInput, setRoomAccessInput] = useState("");
  const [unlockedPrivateRoomIds, setUnlockedPrivateRoomIds] = useState([]);
  ...
  const selectedRoomIsPrivate = selectedRoomObject?.visibility === "private";
  const selectedRoomUnlocked =
    isHost ||
    !selectedRoomIsPrivate ||
    unlockedPrivateRoomIds.includes(Number(selectedRoomId));
  const selectedRoomBlocked = !!selectedRoomObject && !selectedRoomUnlocked;
  const canUseMediaControls = isHost && selectedRoomUnlocked;
  const canUseAdminControls = isHost;
  ...
  function syncSelectedRoom(roomsList, preferredId = null) {
    if (!Array.isArray(roomsList) || roomsList.length === 0) {
      ...
      setRoomFormName("");
      setRoomFormMeta("");
      setRoomFormVisibility("public");
      setRoomFormAccessCode("");
      return;
    }
    ...
    setRoomFormVisibility(targetRoom.visibility || "public");
    setRoomFormAccessCode(targetRoom.accessCode || "");
    setRoomAccessInput("");
  }
  ...
  async function loadRooms(preferredId = null) {
    try {
      ...
      setDataError("");
      const response = await fetch(`${API_BASE}/api/rooms?role=${userRole}`);
  ...
  async function loadChat({ silent = false } = {}) {
    ...
    try {
      ...
      setDataError("");
      const response = await fetch(`${API_BASE}/api/chat?roomId=${selectedRoomId}`);
  ...
  async function loadBulletin({ silent = false } = {}) {
    if (!selectedRoomId) {
      setBulletinText("Convention announcements will appear here after you load a text file.");
      setBulletinFileName("");
      setBulletinStatus("No bulletin file loaded.");
      if (!silent) setBulletinLoading(false);
      return;
    }

    try {
      if (!silent) setBulletinLoading(true);
      setDataError("");
      const response = await fetch(`${API_BASE}/api/bulletin?roomId=${selectedRoomId}`);
      if (!response.ok) throw new Error("Could not load bulletin.");
      const data = await response.json();
  ...
  async function handleCreateRoom() {
    ...
    const visibility = roomFormVisibility === "private" ? "private" : "public";
    const accessCode = roomFormAccessCode.trim();
    ...
    if (visibility === "private" && !accessCode) {
      setStatusText("Private rooms require an access code");
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/api/rooms`, {
        ...
        body: JSON.stringify({ name, meta, visibility, accessCode }),
      });
  ...
  function handleNewRoomDraft() {
    ...
    setRoomFormAccessCode("");
    setRoomAccessInput("");
    setLeftPanelTab("admin");
    setStatusText("New room draft ready");
  }

  function handleUnlockPrivateRoom() {
    if (!selectedRoomObject || !selectedRoomIsPrivate) return;

    const expected = String(selectedRoomObject.accessCode || "").trim();
    const entered = roomAccessInput.trim();

    if (!expected) {
      setStatusText("This room has no access code configured");
      return;
    }

    if (entered !== expected) {
      setStatusText("Incorrect room access code");
      return;
    }

    setUnlockedPrivateRoomIds((current) => {
      const id = Number(selectedRoomObject.id);
      return current.includes(id) ? current : [...current, id];
    });
    setStatusText(`Private room unlocked: ${selectedRoomObject.name}`);
    setRoomAccessInput("");
  }

  async function startCamera() {
    if (!canUseMediaControls) {
      setStatusText(isHost ? "Unlock the private room first" : "Host mode required");
      return;
    }
    ...
    try {
      ...
      const message = error?.message || "Camera could not be started.";
  ...
  async function startShare() {
    ...
    try {
      ...
      const message = error?.message || "Screen share could not be started.";
  ...
  function handleEndLive() {
    if (!isHost) {
      setStatusText("Host mode required");
      return;
    }
  ...
  function handleSelectRoom(room) {
    ...
    setRoomFormVisibility(room.visibility || "public");
    setRoomFormAccessCode(room.accessCode || "");
    setRoomAccessInput("");
  ...
  async function saveBulletinToServer(fileName, text) {
    const response = await fetch(`${API_BASE}/api/bulletin`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ roomId: selectedRoomId, fileName, text }),
    });

    if (!response.ok) throw new Error("Could not save bulletin.");
  ...
  function handleBulletinFileChange(event) {
    ...
    reader.onload = async function (loadEvent) {
      ...
      try {
        ...
        setStatusText(`Bulletin loaded into ${selectedRoom}: ${file.name}`);
      } catch (error) {
  ...
  async function handleSendMessage() {
    ...
    if (selectedRoomBlocked) {
      setStatusText("Unlock the private room before chatting");
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/api/chat`, {
        ...
        body: JSON.stringify({
          ...
          author: displayName.trim() || (isHost ? "Host User" : "Viewer"),
      ...
      setChatInput("");
      setStatusText(`Message sent to ${selectedRoom} chat`);
  ...
  useEffect(() => {
    async function attachStreamToVideo(videoRef, stream) {
      ...
      try {
        ...
        const message = error?.message || "Video preview could not start.";
        setCameraError(message);
  ...
  useEffect(() => {
    async function attachShareToVideo(videoRef, stream) {
      ...
      try {
        ...
        const message = error?.message || "Screen share preview could not start.";
        setShareError(message);
        ...
        <header style={heroHeader}>
          <div style={heroTopRow}>
            <div style={heroContent}>
              ...
              <p style={heroText}>
                Per-room chat and per-room bulletin separation are now built into the
                stage-first layout, while host/viewer roles, private room access,
                room admin, and media controls stay intact.
              </p>
            </div>

            <div style={heroUtilityColumn}>
              ...
              <div style={heroModePill(isHost ? "green" : "slate")}>
                {isHost ? "Host Mode" : "Viewer Mode"}
              </div>
            </div>
          </div>

          <div style={heroBottomRow}>
            ...
            <div style={controlCluster}>
              <button onClick={handleGoLive} style={buttonPrimary} disabled={!canUseMediaControls}>
                Go Live
              </button>
              <button onClick={handleEndLive} style={buttonSecondary} disabled={!isHost}>
                End Live
              </button>
              <button onClick={startCamera} style={buttonPurple} disabled={!canUseMediaControls}>
                Start Camera
              </button>
              <button onClick={stopCamera} style={buttonSecondary} disabled={!isHost}>
                Stop Camera
              </button>
              <button onClick={startShare} style={buttonBlue} disabled={!canUseMediaControls}>
                Start Share
              </button>
              <button onClick={stopShare} style={buttonSecondary} disabled={!isHost}>
                Stop Share
              </button>
            </div>
          </div>
          ...
          <aside style={leftRail}>
            <div style={panelHeaderRow}>
              <div>
                ...
                <div style={smallMutedText}>
                  Rooms live on one tab. Admin or access controls live on the other.
                </div>
              </div>

              <div style={miniBadge}>
                {roomsLoading ? "Loading..." : `${visibleRooms.length} total`}
              </div>
            </div>
            ...
            <div style={leftTabBar}>
              <button
                ...
                style={leftTabButton(leftPanelTab === "rooms")}
              >
                Rooms
              </button>
              <button
                ...
                style={leftTabButton(leftPanelTab === "admin")}
              >
                {isHost ? "Admin" : "Access"}
              </button>
            </div>
            ...
            {leftPanelTab === "rooms" ? (
              <div style={leftPanelSection}>
                <div style={roomListWrap}>
                  {visibleRooms.map((room) => {
                    const active = selectedRoom === room.name;
                    const locked = !isHost && room.visibility === "private";
                    ...
                    return (
                      ...
                      >
                        <div style={roomCardTopRow}>
                          ...
                          <div style={roomRightWrap}>
                            {room.visibility === "private" ? (
                              <span style={lockBadge}>Private</span>
                            ) : (
                        ...
                        {locked ? (
                          <div style={lockedHint}>Viewer access requires room code</div>
                        ) : null}
                      </button>
                    );
                  })}
              ...
              <div style={leftPanelSection}>
                {canUseAdminControls ? (
                  <div style={roomAdminPanel}>
                    ...
                    <select
                      value={roomFormVisibility}
                      onChange={(event) => setRoomFormVisibility(event.target.value)}
                      style={textInput}
                    >
                    ...
                    {roomFormVisibility === "private" ? (
                      <input
                        value={roomFormAccessCode}
                        onChange={(event) => setRoomFormAccessCode(event.target.value)}
                        placeholder="Private room access code"
                        style={textInput}
                      />
                    ) : null}
                  ...
                  <div style={viewerInfoPanel}>
                    <div style={sectionLabel}>Viewer Access</div>
                    <div style={smallMutedText}>
                      Viewer mode can unlock private rooms, read room-specific bulletins,
                      and chat inside the currently selected room.
                    </div>

                    {selectedRoomIsPrivate ? (
                      <div style={viewerAccessCard}>
                        <div style={fieldLabel}>Selected private room</div>
                        <div style={viewerRoomName}>{selectedRoom}</div>
                        <input
                          value={roomAccessInput}
                          onChange={(event) => setRoomAccessInput(event.target.value)}
                          placeholder="Enter room access code"
                          style={textInput}
                        />
                        <button type="button" onClick={handleUnlockPrivateRoom} style={buttonPrimary}>
                          Unlock Room
                        </button>
                      </div>
                    ) : (
                      <div style={viewerAccessCard}>
                        <div style={smallMutedText}>
                          Public rooms open normally. Select a private room to enter its access code here.
                        </div>
                      </div>
                    )}
                  </div>
          ...
          <div style={centerColumn}>
            <section style={stagePanel}>
              ...
              <div style={stageWrap}>
                ...
                {selectedRoomBlocked ? (
                  <div style={privateGateScreen}>
                    <div style={privateGateTitle}>Private room access required</div>
                    <div style={privateGateText}>
                      This room is private. Enter the room access code to unlock this stage as a viewer.
                    </div>

                    <div style={privateGateForm}>
                      <input
                        value={roomAccessInput}
                        onChange={(event) => setRoomAccessInput(event.target.value)}
                        placeholder="Enter room access code"
                        style={privateCodeInput}
                      />
                      <button type="button" onClick={handleUnlockPrivateRoom} style={buttonPrimary}>
                        Unlock Room
                      </button>
                  ...
                  <div style={stageInfoScreen}>
                    ...
                    <div style={stageInfoText}>
                      {isHost
                        ? "This room now carries its own chat and bulletin feed in addition to the existing stage tools."
                        : "Viewer mode is active. Watch the stage, read this room’s bulletin, use this room’s chat, and unlock private rooms when needed."}
                    </div>
                ...
                {showFloatingCamera && !selectedRoomBlocked ? (
                ...
                {showDockShare && !selectedRoomBlocked ? (
                  <button
                    type="button"
                ...
                {showExpandedShare && !selectedRoomBlocked ? (
                  <button
                    type="button"
                    onClick={toggleShareView}
              ...
              <div style={contentPanel}>
                <div style={panelHeaderRow}>
                  <div>
                    ...
                    <div style={smallMutedText}>
                      This feed now belongs to <strong>{selectedRoom || "the selected room"}</strong>.
                    </div>
                  </div>
                ...
                <div style={chatComposer}>
                  <textarea
                    ...
                    placeholder={
                      selectedRoomBlocked
                        ? "Unlock the private room before sending chat..."
                        : `Type a message for ${selectedRoom || "this room"}...`
                    }
                    rows={3}
                    style={chatTextarea}
                    disabled={selectedRoomBlocked || !selectedRoomId}
                  />
                  ...
                  <button
                    ...
                    style={buttonPrimary}
                    disabled={chatSending || selectedRoomBlocked || !selectedRoomId}
                  >
          ...
          <aside style={rightRail}>
            <div style={panelHeaderRow}>
              <div>
                ...
                <div style={smallMutedText}>
                  This bulletin now belongs to <strong>{selectedRoom || "the selected room"}</strong>.
                </div>
              </div>
            ...
            <button
              onClick={handleBulletinOpen}
              style={buttonPurple}
              disabled={!isHost || !selectedRoomId}
            >
              Choose Bulletin File
            </button>

            <div style={bulletinAlert}>{bulletinStatus}</div>
            ...
            <div style={bulletinBoardWrap}>
              <div style={bulletinTickerHeader}>
                ...
                {selectedRoom || "Convention Feed"}
              </div>

              <div style={bulletinContent}>{bulletinText}</div>
            </div>
          </aside>
        </section>
      </div>
...
const mainGrid = {
  display: "grid",
  gridTemplateColumns: "300px minmax(0, 1fr) 350px",
  gap: "18px",
  alignItems: "start",
};
...
const leftTabButton = (active) => ({
  minHeight: "40px",
  padding: "10px 12px",
  borderRadius: "14px",
  border: active ? "1px solid rgba(96,165,250,0.45)" : "1px solid rgba(255,255,255,0.08)",
  background: active
    ? "linear-gradient(180deg, rgba(37,99,235,0.30), rgba(30,41,59,0.38))"
...
const privateGateTitle = {
  fontSize: "30px",
  fontWeight: 900,
  color: "#ffffff",
};

const privateGateText = {
  fontSize: "15px",
  lineHeight: 1.8,
  color: "#f8e7b2",
  maxWidth: "720px",
